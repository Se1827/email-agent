          ...
          <div className="provider-presets">
            {['gmail', 'outlook', 'yahoo', 'imap', 'mock'].map(p => (
              <button key={p} type="button" className={`preset-btn ${form.provider === p ? 'active' : ''}`} onClick={() => applyPreset(p)}>
                {p === 'gmail' ? '📧' : p === 'outlook' ? '📨' : p === 'yahoo' ? '📩' : p === 'mock' ? '🧪' : '🔌'} {p.charAt(0).toUpperCase() + p.slice(1)}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="stab">
      ...
      <div className="s-section">
        ...
        <p className="s-hint">Get one free at <a href="https://console.groq.com" target="_blank" rel="noreferrer">console.groq.com</a>. For Gmail, use an App Password, not your account password.</p>
      </div>
      ...
      <div className="s-section">
        ...
        <p className="s-hint">llama-3.3-70b-versatile is recommended for best classification quality.</p>
      </div>
